...
import com.triompha.dao.ds.DdsContext;
...
public class HibernateDaoImpl extends HibernateDaoSupport implements BaseDao {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <T> List<T> listPage(final String sql, final int start, final int size, final Object... values) {
        this.determineDs();
    ...
    public static void main(String[] args) {
        ...
        BaseDao baseDao = (BaseDao) app.getBean("baseDao");
        
        
        List<Long> list =	baseDao.list("select uid from Enterprise where stat=? and logourl=?", 1,"asdf");
        System.out.println(list);
//        ((HibernateDaoImpl)baseDao).getSessionFactory().getClassMetadata("asdf").getIdentifierPropertyName()
//        
...
//        hibernateConf.buildSessionFactory();//注解方式必须的  
        Map map =	((HibernateDaoImpl)baseDao).getSessionFactory().getAllClassMetadata();
        for(String s : (Set<String>)map.keySet()){
        	SingleTableEntityPersister per = (SingleTableEntityPersister) map.get(s);
        	System.out.println(per.getIdentifierColumnNames());
        	System.out.println(per.getIdentifierGenerator());
        }
        
        
//        
//		System.out.println( ((HibernateDaoImpl)baseDao).getSessionFactory().getAllClassMetadata());;
//		System.out.println( ((HibernateDaoImpl)baseDao).getSessionFactory().getAllCollectionMetadata());
//		System.out.println( ((HibernateDaoImpl)baseDao).getSessionFactory().getClassMetadata("Enterprise"));
//		
//		System.out.println( ((HibernateDaoImpl)baseDao).getSessionFactory().getStatistics());
